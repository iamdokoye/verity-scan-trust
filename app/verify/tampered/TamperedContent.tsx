...
export function TamperedContent() {
  ...
  const status = searchParams.get("status") ?? "tampered";

  const label =
    status === "invalid_signature" ? "Invalid Signature" : "Tampered";
  const description =
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        <p className="mb-6 text-sm leading-relaxed text-foreground">
          The document presented does not match the original record. The content
          may have been altered after issuance.{" "}
        ...
        <div className="mt-6 rounded-md border border-warning/40 bg-warning/15 p-4 text-sm text-foreground">
          ...
          directly to verify the document&apos;s authenticity.
        </div>
