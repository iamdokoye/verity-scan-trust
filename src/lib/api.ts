 ...
 * Votta API client
 *
 * All requests go through the Express backend at VITE_API_URL.
 ...
 *   const doc  = await api.post('/documents/students/:id', { body: formData });
 */

const BASE_URL = (import.meta.env.VITE_API_URL as string) ?? 'http://localhost:3000/api/v1';

// Token is stored in memory for the session + localStorage for page reloads
...
async function request<T>(
  ...
  // Our API always returns { success: true, data: ..., meta?: ... }
...
export async function apiLogin(email: string, password: string): Promise<LoginResponse> {
  ...
  try { localStorage.setItem('votta_refresh_token', data.refreshToken); } catch {}
  return data;
}
...
export async function apiRefresh(): Promise<string | null> {
  const refreshToken = localStorage.getItem('votta_refresh_token');
...
export async function apiGetMe() {
  return api.get<{ id: string; email: string; role: string; institutionId: string }>('/auth/me');
}
