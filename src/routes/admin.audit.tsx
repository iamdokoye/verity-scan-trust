  ...
  return (
    <div>
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
        ...
        <Input placeholder="Search actor or target…" className="lg:col-span-2" />
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
          ...
          <tbody className="divide-y divide-border">
            {auditLog.map((e, i) => (
              <tr key={i}>
                ...
                <td className="px-5 py-3">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-medium ${ACTION_COLORS[e.action] || "bg-muted text-muted-foreground"}`}>
                    {e.action}
                  </span>
                </td>
        ...
        <div className="flex items-center justify-between border-t border-border px-5 py-3 text-xs text-muted-foreground">
          <span>Showing 1–{auditLog.length} of {auditLog.length}</span>
          <div className="flex gap-1">
            <button className="rounded-md border border-border px-2.5 py-1 hover:bg-muted">Previous</button>
            <button className="rounded-md border border-border px-2.5 py-1 hover:bg-muted">Next</button>
          </div>
        </div>
      </div>
    </div>
