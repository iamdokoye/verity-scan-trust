...
export const Route = createFileRoute("/admin/documents/upload")({
  component: UploadDoc,
});
...
function UploadDoc() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-1 text-2xl font-semibold tracking-tight text-foreground">Upload and Secure Document</h1>
      ...
      <div className="space-y-6 rounded-lg border border-border bg-card p-6">
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-foreground">File</label>
          <label className="flex cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed border-border bg-muted/40 p-10 text-center transition-colors hover:bg-muted">
            ...
            <div className="mt-1 text-xs text-muted-foreground">Max file size: 5MB</div>
            <input
              type="file"
        ...
        <div>
          <Button className="w-full" size="lg">Upload and Sign</Button>
          <p className="mt-3 text-center text-xs text-muted-foreground">
            Once uploaded, this document will be cryptographically hashed and signed with the
            institution's private key. The process is automatic and takes less than 3 seconds.
          </p>
        </div>
      </div>
    </div>
