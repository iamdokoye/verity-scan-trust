...
function StudentsList() {
  return (
    <div>
      <PageTitle
        title="Students"
        action={<Button><UserPlus className="h-4 w-4" /> Add Student</Button>}
      />
          ...
          <tbody className="divide-y divide-border">
            {studentList.map((s) => (
              <tr key={s.matric} className="hover:bg-muted/30">
                <td className="px-5 py-3 font-mono text-foreground">{s.matric}</td>
                <td className="px-5 py-3 font-medium text-foreground">{s.name}</td>
                <td className="px-5 py-3 text-foreground">{s.dept}</td>
                <td className="px-5 py-3 text-center text-foreground">{s.year}</td>
                <td className="px-5 py-3 text-center font-semibold text-foreground">{s.cgpa.toFixed(2)}</td>
                <td className="px-5 py-3 text-right">
                  <Link
                    to="/admin/students/$matric"
                    params={{ matric: s.matric }}
