    ...
    <div>
      ...
      <div className="mb-6 flex flex-col items-start gap-4 rounded-lg border border-border bg-card p-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="mt-1 text-5xl font-bold tracking-tight text-primary">{student.cgpa.toFixed(2)}</div>
        </div>
                ...
                <div className="border-t border-border">
                  <table className="w-full text-sm">
                    <thead className="bg-muted/50 text-xs uppercase tracking-wider text-muted-foreground">
                      <tr>
                        <th className="px-5 py-2 text-left font-medium">Course Code</th>
                        <th className="px-5 py-2 text-left font-medium">Course Title</th>
                    ...
                    <tbody>
                      {s.courses.map((c, idx) => (
                        <tr key={c.code} className={idx % 2 === 0 ? "bg-background" : "bg-muted/30"}>
                          <td className="px-5 py-2.5 font-mono text-foreground">{c.code}</td>
                          <td className="px-5 py-2.5 text-foreground">{c.title}</td>
                          <td className="px-5 py-2.5 text-center text-foreground">{c.units}</td>
                          <td className="px-5 py-2.5 text-center font-semibold text-foreground">{c.grade}</td>
                          <td className="px-5 py-2.5 text-center text-foreground">{c.point}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="border-t border-border bg-muted/40 text-xs">
                      <tr>
                        <td colSpan={2} className="px-5 py-3 text-right font-medium text-muted-foreground">
                          Total Credit Units: <span className="text-foreground">{totalUnits}</span>
                        </td>
                        <td className="px-5 py-3 text-center text-muted-foreground">
                          Weighted: <span className="text-foreground">{weighted}</span>
                        </td>
                        <td colSpan={2} className="px-5 py-3 text-center text-muted-foreground">
                          GPA: <span className="font-semibold text-foreground">{s.gpa.toFixed(2)}</span>
                        </td>
                      </tr>
                    </tfoot>
                  </table>
